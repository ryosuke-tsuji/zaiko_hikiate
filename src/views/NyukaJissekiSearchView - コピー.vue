<template>
  <div>
    ...
    <v-container fluid>
      ...
      <v-row>
        <v-col class="d-flex justify-end mr-10">
          <v-btn class="mr-10 mb-3" large @click.stop="setting"><span>設定</span></v-btn>
          <v-btn class="mr-10 mb-3" large href="nyuka_dtl"><span>詳細</span></v-btn>
      ...
      <v-row>
        <v-col>
          <v-card>
            ...
            <v-card-text>固定部のclassを特別なものにしている。動的にどうするか <input type="number" min="1" style="width:50px; text-align:right;" v-model="ptn1Cnt" @change="dynamicClassChange">
              <div style="overflow-x: scroll; text-align: left; padding: 0px; margin: 0px;">
                <table id="PTN1">
                  <tr>
                    <th class="fixed01">test1</th>
                    <th>test2</th>
                    <th>test3</th>
                    <th>test4</th>
                    <th>test5</th>
                  ...
                  <tr>
                    <td class="fixed01">1234</td>
                    <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                  </tr>
                  <tr>
                    <td class="fixed01">2345</td>
                    <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                    <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                    <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                    <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                  </tr>
    ...
    <v-container fluid>
      <v-row>
        <v-col>
          <v-card>
            <v-card-title>HTML + CSS2</v-card-title>
            <v-card-text>固定部のclassを特別なものにしている。動的にどうするか <input type="number" min="1" style="width:50px; text-align:right;" v-model="ptn1Cnt" @change="dynamicClassChange">
              <div style="overflow-x: scroll; text-align: left; padding: 0px; margin: 0px;">
                <table id="PTN1">
                  <tr>
                    <th :class="getDynamicClass(1)">test1</th>
                    <th :class="getDynamicClass(2)">test2</th>
                    <th :class="getDynamicClass(3)">test3</th>
                    <th :class="getDynamicClass(4)">test4</th>
                    <th :class="getDynamicClass(5)">test5</th>
                  </tr>
                  <tr>
                    <td :class="getDynamicClass(1)">1234</td>
                    <td :class="getDynamicClass(2)">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    <td :class="getDynamicClass(3)">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    <td :class="getDynamicClass(4)">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    <td :class="getDynamicClass(5)">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                  </tr>
                  <tr>
                    <td :class="getDynamicClass(1)">1234</td>
                    <td :class="getDynamicClass(2)">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
            ...
            <v-card-text>固定部のclassを特別なものにしている。動的にどうするか <input type="number" min="1" style="width:50px; text-align:right;" v-model="ptn1Cnt" @change="dynamicClassChange">
              <div class="table-wrapper">
                <table class="table text-nowrap sticky-table table-borderless">
                  ...
                  <tbody>
                    <tr>
                      <td class="fixed-column-0">1234</td>
                      <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                      <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                      <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                      <td id="PTN1">aaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaaa</td>
                    ...
                    <tr>
                      <td class="fixed-column-0">2345</td>
                      <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                      <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                      <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
                      <td id="PTN1">bbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbbb</td>
    ...
    <v-container fluid>
      <v-row>
        <v-col>
          <div>v-data-table 2つ並べる フッタどうする？ ソートすると左右ずれる<input type="number" min="1" style="width:50px; text-align:right;" v-model="ptn1Cnt"></div>
          <v-card style="display:flex" width="1000px">
            <v-data-table :headers="shownHeadersLeft" :items="itemListLeft" item-key="radioButtonID" dense multi-sort fixed-header hide-default-footer no-data-text="検索してください。" height=400>
              <template #[`item.radioButtonID`]="{ item }">
                <input :id="'Radio_' + item.radioButtonID" type="radio" :value="item.radioButtonID" v-model="selectItemList">
              </template>
            </v-data-table>
            <v-data-table :headers="shownHeadersRight" :items="itemListRight" item-key="radioButtonID" dense multi-sort fixed-header no-data-text="検索してください。" height=400>
              <template #[`item.radioButtonID`]="{ item }">
                <input :id="'Radio_' + item.radioButtonID" type="radio" :value="item.radioButtonID" v-model="selectItemList">
              </template>
            </v-data-table>
...
export default {
  ...
  methods: {
    ...
    dynamicClassChange() {
      ...
      this.height = 0;
      for (var i = 0; i < 2; i++) {
        (".fixed-header-" + i + " th").css('top', this.header);
        this.height += (".fixed-header-" + i + " th").outerHeader();
      }
    },
  ...
  computed: {
    ...
    shownHeadersLeft() {
      return this.headers.filter(value => {
        if (value.shown && value.displayOrder <= this.ptn1Cnt) {
          return true;
        }
        return false;
      });
    ...
    itemListLeft() {
      return this.itemList;//.filter(value => {if(value.index < 3) return true;});
    },
    itemListRight() {
      return this.itemList;//.filter(value => {if(value.index >= 3) return true;});
    },
